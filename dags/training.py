...
try:
    ...
    if SENTRY_DSN:
        ...
        print("Sentry initialized for training pipeline")
except ImportError:
...
def load_features_from_feast():
    ...
    try:
        ...
        print(
            "Loading processed features from MongoDB (X_train_final and X_test_final)..."
        )
        return load_features_from_mongodb()
...
def load_features_from_mongodb():
    ...
    try:
        ...
        if "isFraud" not in X_train.columns:
            ...
            X_train["isFraud"] = np.random.choice(
                [0, 1], size=len(X_train), p=[0.9, 0.1]
            )
...
def prepare_training_data():
    ...
    try:
        ...
        print(
            f"Training columns: {list(X_train.columns)[:10]}..."
        ...
        train_numeric_columns = X_train_features.select_dtypes(
            include=[np.number]
        ).columns
    ...
    try:
        ...
        with mlflow.start_run(run_name="xgboost_fraud_detection"):
            ...
            model_info = mlflow.xgboost.log_model(
                model, "model", registered_model_name="fraud_detection_xgboost"
            )
    ...
    try:
        ...
        with mlflow.start_run(run_name="lightgbm_fraud_detection"):
            ...
            model.booster_.save_model(
                os.path.join(MODEL_DIR, "lightgbm_fraud_model.txt")
            )
    ...
    try:
        ...
        with mlflow.start_run(run_name="isolation_forest_fraud_detection"):
            ...
            auc_score = roc_auc_score(
                y_val, -y_scores
            )  # Negative because lower scores indicate outliers
            ...
            model_info = mlflow.sklearn.log_model(
                model, "model", registered_model_name="fraud_detection_isolation_forest"
            )
...
def select_best_model():
    ...
    try:
        ...
        for model in models_performance:
            print(
                f"{model['model']}: AUC={model['auc']:.4f}, F1={model['f1_score']:.4f}"
            )
        ...
        with mlflow.start_run(run_name="best_model_selection"):
            ...
            mlflow.log_metrics(
                {"best_f1_score": best_model["f1_score"], "best_auc": best_model["auc"]}
            )
...
with DAG(
    ...
    test_mlflow_task = PythonOperator(
        task_id="test_mlflow_connection", python_callable=test_mlflow_connection
    )

    # Load features
    load_features_task = PythonOperator(
        task_id="load_features", python_callable=load_features_from_feast
    )

    # Prepare training data
    prepare_data_task = PythonOperator(
        task_id="prepare_training_data", python_callable=prepare_training_data
    )

    # Train models
    train_xgboost_task = PythonOperator(
        task_id="train_xgboost", python_callable=train_xgboost_model
    )

    train_lightgbm_task = PythonOperator(
        task_id="train_lightgbm", python_callable=train_lightgbm_model
    )

    train_isolation_forest_task = PythonOperator(
        task_id="train_isolation_forest", python_callable=train_isolation_forest
    )

    # Select best model
    select_best_model_task = PythonOperator(
        task_id="select_best_model", python_callable=select_best_model
    )
