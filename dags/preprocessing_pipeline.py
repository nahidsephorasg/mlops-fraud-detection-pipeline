...
try:
    ...
    if SENTRY_DSN:
        ...
        print("Sentry initialized for preprocessing pipeline")
except ImportError:
...
def load_data():
    ...
    try:
        ...
        if train_transaction.empty or test_transaction.empty:
            raise ValueError(
                "No data found in MongoDB collections. Please import the dataset first using dataset_import.sh"
            )

        # Print column names to debug
        print(f"train_transaction columns: {list(train_transaction.columns[:5])}")
        print(
            f"train_identity columns: {list(train_identity.columns[:5]) if not train_identity.empty else 'Empty'}"
        )

        # Check if TransactionID exists
        if "TransactionID" not in train_transaction.columns:
            raise KeyError(
                f"TransactionID column not found. Available columns: {list(train_transaction.columns[:10])}"
            )

        # Merge datasets
        X_train = (
            train_transaction.merge(train_identity, on="TransactionID", how="left")
            if not train_identity.empty
            else train_transaction
        )
        X_test = (
            test_transaction.merge(test_identity, on="TransactionID", how="left")
            if not test_identity.empty
            else test_transaction
        )
...
def normalize_d_columns_task():
    ...
    try:
        ...
        X_train["TransactionDT"] = pd.to_numeric(
            X_train["TransactionDT"], errors="coerce"
        )
        X_test["TransactionDT"] = pd.to_numeric(
            X_test["TransactionDT"], errors="coerce"
        )
        ...
        for col in d_columns:
            ...
            # Apply normalization
            X_train.loc[train_mask, col] = (
                X_train.loc[train_mask, col] - X_train.loc[train_mask, "TransactionDT"]
            )
            X_test.loc[test_mask, col] = (
                X_test.loc[test_mask, col] - X_test.loc[test_mask, "TransactionDT"]
            )
    ...
    try:
        ...
        for df in [X_train, X_test]:
            if "TransactionAmt" in df.columns:
                ...
                df["TransactionCents"] = df["TransactionAmt"] - df[
                    "TransactionAmt"
                ].astype(int)
    ...
    try:
        ...
        if "event_timestamp" not in data.columns:
            ...
            data["event_timestamp"] = [
                base_time + timedelta(seconds=i) for i in range(len(data))
            ]
...
def call_simple_feast_api(data):
    ...
    try:
        ...
        if store_response.status_code == 200:
            ...
            print(
                f"✓ Successfully stored {response_data.get('records_count', 'unknown')} records"
            )
            return True
        else:
...
with DAG(
    ...
    test_feast_task = PythonOperator(
        task_id="test_feast_connection", python_callable=test_feast_connection
    )
    ...
    normalize_d_columns = PythonOperator(
        task_id="normalize_d_columns", python_callable=normalize_d_columns_task
    )

    frequency_encoding = PythonOperator(
        task_id="frequency_encoding", python_callable=frequency_encoding_task
    )

    combine_features = PythonOperator(
        task_id="combine_features", python_callable=combine_features_task
    )

    label_encoding = PythonOperator(
        task_id="label_encoding", python_callable=label_encoding_task
    )

    group_aggregation = PythonOperator(
        task_id="group_aggregation", python_callable=group_aggregation_task
    )
    ...
    transaction_cents = PythonOperator(
        task_id="transaction_cents", python_callable=transaction_cents_task
    )
    ...
    remove_columns = PythonOperator(
        task_id="remove_columns", python_callable=remove_columns_task
    )

    # Push to simple Feast server
    push_to_feast_task = PythonOperator(
        task_id="push_to_feast", python_callable=push_to_feast
    )
    ...
    (
        ...
        >> label_encoding
        >> group_aggregation
        >> group_aggregation_nunique
        >> transaction_cents
        >> frequency_encoding_combined
        >> remove_columns
        >> push_to_feast_task
    )
